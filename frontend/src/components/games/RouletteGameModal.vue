          ...
          <div class="flex flex-col gap-6 w-full max-w-md">
            ...
            <div v-if="lastResult" class="text-center animate-in fade-in slide-in-from-top-2">
              <div class="text-lg font-bold uppercase tracking-widest"
                   :class="lastResult.includes('won') ? 'text-green-400 drop-shadow-[0_0_10px_rgba(34,197,94,0.8)]' : 'text-red-500'">
                {{ lastResult }}
              </div>
            </div>
...
async function spin() {
  ...
  try {
    ...
    // Backend: if (color == randomNumber%2) -> Parzyste (0) to Black, Nieparzyste (1) to Red.
    ...
    const winningNumber = data.randomNumber; // np. 17
    const gain = data.gain; // np. 0 lub 500
    ...
    if (wheelEl) {
      ...
      setTimeout(async () => {
        ...
        // Odświeżamy portfel (backend już zaktualizował bazę)
        await auth.fetchBalance();

        if (gain > 0) {
          lastResult.value = `VICTORY! Result: ${winningNumber}. YOU WON $${gain}!`;
          // Opcjonalnie: dźwięk wygranej
        } else {
          lastResult.value = `DEFEAT. Result: ${winningNumber}.`;
        }
    ...
    console.error(error);
    isSpinning.value = false;
    lastResult.value = 'Network Error. Try again.';
    // W przypadku błędu (np. brak środków) odświeżamy balans, by pokazać prawdę
    await auth.fetchBalance();
  }
}
</script>
...
.neon-roulette-cap {
  ...
  background: #111;
  border: 2px solid #b84ff6;
  border-radius: 50%;
  box-shadow: 0 0 20px #b84ff6;
  z-index: 40;
}
...
/* --- TABLE HUD --- */
.table-btn {
  padding: 0.75rem 1.5rem; border-radius: 0.5rem; font-weight: 800; border: 1px solid;
  transition: all 0.2s; text-transform: uppercase; font-size: 0.8rem;
}
.table-btn.active { box-shadow: 0 0 15px currentColor; transform: scale(1.05); border-color: white; }

.number-btn {
  height: 40px; border-radius: 4px; font-weight: bold; border: 1px solid; transition: all 0.1s;
}
.number-btn:hover { transform: translateY(-2px); }
.number-btn.active { background: #fff !important; color: #000 !important; box-shadow: 0 0 15px #fff; border-color: #fff; }
</style>